...
class Channel_object():
    def __init__(self, ch_name, SRC_ip_addr, SRC_port, SRC_username, SRC_userpass, OUT_ip_addr, OUT_port, OUT_username, OUT_userpass, SRC_node, SRC_database, SRC_measurement, model_name, model_dir, rate, OUT_node, OUT_database, OUT_measurement, ML_core_ip_addr, ML_core_port, ML_core_type):
    ...
    def get_raw_data_from_source(self):
        ...
        if (self.ML_core_type == "tf-core"):

            query_body = "SELECT " + self.ch_name + " from " + self.SRC_measurement + " WHERE time > now() - 5m;"

            self.send_data_as_df = self.LinkAgentIN.query(query_body)     # <<<<<<<<   type of returned data from Influx is data frame
    ...
    def send_raw_data_to_ml(self):
        ...
        if (self.ML_core_type != "tf-core"):           #<<<<<<<<<<<<<<<<<<< this part for models in flask
            ...
            tf.app.flags.DEFINE_string('server', '{0}:{1}'.format(self.ML_core_ip_addr, self.ML_core_port),
            ...
            prediction = stub.Predict(request, 10.0)  # 5 secs timeout              # <<<<<<<<<<<<<<< ERRRRROOOORRRRRR
            ...
            json_influx = [
                {
                    ...
                    "time": self.current_time,
                    "fields": {
                        str(self.ch_name + "_predicted_tf"): putting_data_in_list[0] * 0.95,
                        str(self.ch_name + "_mse_tf"): putting_data_in_list[1] * 1.05,
                        str(self.ch_name + "_live"): putting_data_in_list[2]
                    }
                }
            ]
